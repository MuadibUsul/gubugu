...
`server/recognition/service.ts` 调用 `buildMockRecognitionCandidates`，候选结果由图片体积等属性推导得出，**与图片内容无关**。拍照、取景、上传、候选确认这套 UI 是真的，但排序结果没有意义。

它在 `AGENTS.md` 里属于 V1 必选项。上线前需要决定：接入真实的相似度检索，或者在界面上明确标注为实验功能。
