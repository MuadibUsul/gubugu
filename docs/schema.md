...
The camera flow should integrate with schema as an entry workflow, not a model platform.

Recommended support tables or fields:
...
- store ranked candidate SKUs returned by a similarity service
- store final user-confirmed SKU selection

The system should treat recognition as:
...
It should not treat recognition as guaranteed automatic identification.
